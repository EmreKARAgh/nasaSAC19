...
public class TrackManager : MonoBehaviour
{
    ...
    int altSinir=0;
    int ustSinir=100;
    public TrackSegment suluSegment;
    ...
    public void SpawnNewSegment()
    {
        ...
        suluSegment = m_CurrentThemeData.zones[0].prefabList[2];
        if (segmentToUse == suluSegment)
        {
            segmentToUse = suluSegment = m_CurrentThemeData.zones[0].prefabList[1];
        }
        if (m_Score >= 2050) { 
            int segmentValue = Random.Range(altSinir,ustSinir);
            if(segmentValue == 50)
            {
                segmentToUse = suluSegment;
            }
        }
    ...
    public void SpawnObstacle(TrackSegment segment)
    {
        if (segment.possibleObstacles.Length != 0)
        {
            ...
            {
                segment.possibleObstacles[2].Spawn(segment, segment.obstaclePositions[i]);
                //ıf (su getır) {ındex =2}
            }
